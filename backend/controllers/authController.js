...
// const jwt = require("jsonwebtoken");
// const PDFDocument = require("pdfkit");
// const bucket = require("../utils/firebase"); // ✅ Firebase
...
//         const pdfUrl = `https://storage.googleapis.com/${bucket.name}/pdfs/${fileName}`;
...
//     const { personalData, schoolNames } = req.body;
//     if (!schoolNames || schoolNames.length === 0) return res.status(400).json({ message: "No school selected" });

//     const user = await User.findById(req.userId);
//     if (!user) return res.status(404).json({ message: "User not found" });
//     if (user.formSubmitted) return res.status(403).json({ message: "Form already submitted" });
...
//       if (i !== 0) doc.moveTo(leftX, currentY).lineTo(leftX + pageWidth, currentY).stroke();
...
//     doc.rect(leftX, startY, pageWidth, currentY - startY).stroke();
//     doc.moveTo(leftX + pageWidth / 2, startY).lineTo(leftX + pageWidth / 2, currentY).stroke();
...
//     const pdfBuffer = Buffer.concat(chunks);
//     const fileName = `${user.post}_${user.subject}_${user.rollno}`.replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_]/g, "") + ".pdf";
//     const file = bucket.file(`pdfs/${fileName}`);
...
//     res.json({ message: "Form submitted successfully", pdfUrl });

//   } catch (err) {
//     console.error("PDF/Firebase ERROR:", err);
